...
const orderLabel=(x)=>({reservado:'Solicitado',preparado:'Preparado',entregado:'Entregado',cancelado:'Cancelado'}[x]||x||'Solicitado');
...
function openMaterialRequest(materialId='',ctx={}){
  const {members=[],variants=[],items=[],reload=()=>{}}=ctx;
  ...
  const fields=[{name:'socio_id',label:'Alumno',type:'select',required:true,options:options(members,m=>`${m.apellidos||''}, ${m.nombre}`.replace(/^, /,''))},{name:'material_id',label:'Artículo',type:'select',required:true,value:materialId,options:options(activeItems)},{name:'variante_id',label:'Talla / color (opcional)',type:'select',options:[]},{name:'cantidad',label:'Cantidad',type:'number',min:1,value:1},{name:'observaciones',label:'Observaciones',type:'textarea',full:true,placeholder:'Talla aproximada, consulta o detalle para secretaría…'}];
  const modal=openForm({title:'Solicitar material',subtitle:'La solicitud llegará al club y podrás seguir su estado en Mis pedidos.',fields,submitText:'Enviar solicitud',onSubmit:async v=>{await repos.material.request(v);toast('Solicitud de material registrada');await reload();}});
...
function openDirectMaterialDelivery(ctx={}){
  const {members=[],variants=[],items=[],reload=()=>{}}=ctx;
  if(!members.length){toast('No hay alumnos disponibles.','error');return;}
  const activeItems=items.filter(i=>i.activo!==false);
  const fields=[{name:'socio_id',label:'Alumno',type:'select',required:true,options:options(members,m=>`${m.apellidos||''}, ${m.nombre}`.replace(/^, /,''))},{name:'material_id',label:'Artículo',type:'select',required:true,options:options(activeItems)},{name:'variante_id',label:'Talla / color (opcional)',type:'select',options:[]},{name:'cantidad',label:'Cantidad',type:'number',min:1,value:1},{name:'observaciones',label:'Observaciones internas',type:'textarea',full:true}];
  const modal=openForm({title:'Registrar material entregado',subtitle:'Al confirmar se registra la entrega, se crea el cargo pendiente y entra en los avisos de cobro.',fields,submitText:'Registrar entrega y cargo',onSubmit:async v=>{await repos.material.registerDelivery(v);toast('Entrega registrada y cargo generado');await reload();}});
  const materialSelect=modal.form.elements.material_id,variantSelect=modal.form.elements.variante_id;
  const refreshVariants=()=>{const mid=materialSelect.value;const own=variants.filter(v=>v.activa&&v.material_id===mid);variantSelect.innerHTML='<option value="">Sin variante</option>'+own.map(v=>`<option value="${esc(v.id)}">${esc([v.talla,v.color].filter(Boolean).join(' · ')||v.referencia||'Variante')} · stock ${Number(v.stock||0)}</option>`).join('');};
  materialSelect.addEventListener('change',refreshVariants);refreshVariants();
}
...
export async function renderMaterial(){
  ...
  try{
    ...
    const orderRows=orders.map(o=>{const v=variants.find(x=>x.id===o.variante_id);const validation=o.validacion_estado||((o.estado==='entregado')?'validado':'pendiente');const source=({alumno:'Alumno',familia:'Familia',club:'Club'}[o.origen_registro]||'Club');return `<tr><td>${dateFmt(o.creado_en)}<br><small>${esc(source)}</small></td><td>${esc(members.find(m=>m.id===o.socio_id)?.nombre||'—')}</td><td><strong>${esc(items.find(i=>i.id===o.material_id)?.nombre||'—')}</strong>${v?`<small class="muted">${esc([v.talla,v.color].filter(Boolean).join(' · '))}</small>`:''}</td><td>${esc(o.cantidad)}</td><td>${money(o.importe_total)}</td><td>${badge(validation==='validado'?'Validado':validation==='rechazado'?'Rechazado':'Pendiente de validar',validation==='validado'?'ok':validation==='rechazado'?'danger':'warn')}<br><small>${esc(orderLabel(o.estado))}</small></td><td>${canManage?`<div class="row-actions">${validation==='pendiente'&&o.estado!=='cancelado'?`<button class="btn btn-primary btn-sm validate-material" data-id="${esc(o.id)}">Validar y cobrar</button>`:''}${o.estado!=='entregado'?`<button class="btn btn-ghost btn-sm status-order" data-id="${esc(o.id)}">Estado</button>`:''}</div>`:''}</td></tr>`});
    setMainHtml(`${pageHeader(can?'Material':'Tienda del club',can?'Catálogo, entregas, validación y cargos':'Registra el material que has cogido y sigue su validación',`${can?'<button class="btn btn-primary" id="new-material">'+icon('plus',{size:16})+' Nuevo material</button><button class="btn btn-primary" id="direct-material">'+icon('shoppingBag',{size:16})+' Registrar entrega</button>':''}<button class="btn btn-ghost" id="request-material">${icon('shoppingBag',{size:16})} ${canManage?'Crear solicitud':'He cogido material'}</button>`,'Contenido')}<section class="store-hero"><div><span class="page-kicker">Urban Warriors Store</span><h2>Material con trazabilidad.</h2><p>${canManage?'Registra entregas directamente o valida lo declarado por alumnos y familias. La entrega validada genera su cargo financiero.':'Indica el material que has cogido. El club lo validará antes de generar cualquier cargo.'}</p></div><div class="store-hero-mark">${icon('shield',{size:58})}<strong>UW</strong></div></section><div class="material-grid">${products||empty('Sin material')}</div><div style="height:18px"></div>${card(canManage?'Registro de material':'Mi historial de material',orderRows.length?table(['Fecha / origen','Alumno','Artículo','Cant.','Importe','Validación','Acciones'],orderRows):empty('Sin movimientos','Las solicitudes y entregas aparecerán aquí.'))}`);
    const reload=()=>renderMaterial();
    document.getElementById('new-material')?.addEventListener('click',()=>openMaterialForm(undefined,reload));
    document.getElementById('direct-material')?.addEventListener('click',()=>openDirectMaterialDelivery({members,variants,items:visible,reload}));
    document.getElementById('request-material')?.addEventListener('click',()=>openMaterialRequest('',{members,variants,items:visible,reload}));
    bind('.product-open',id=>productDetail(items.find(x=>x.id===id),{variants,members,can,canManage,reload}));
    bind('.validate-material',async(id,el)=>{el.disabled=true;try{await repos.material.validateDelivery(id);toast('Material validado y cargo pendiente generado');await reload();}catch(e){setError(e);el.disabled=false;}});
    bind('.status-order',id=>openForm({title:'Actualizar pedido',fields:[{name:'estado',label:'Estado',type:'select',required:true,options:[{value:'reservado',label:'Solicitado'},{value:'preparado',label:'Preparado'},{value:'entregado',label:'Entregado'},{value:'cancelado',label:'Cancelado'}]}],onSubmit:async v=>{await repos.material.orderStatus(id,v.estado);toast('Pedido actualizado');await reload();}}));
...
export async function renderNotifications(){
  ...
  try{
    ...
    const actionTypes=new Set(['inscripcion','cuota','material','pago','documento','validacion_pago']);
    const category=(n)=>actionTypes.has(n.tipo)?'accion':n.tipo==='reserva_sesion'||n.tipo==='sesion_cambio'||n.tipo==='clase'?'sesiones':n.tipo==='comunidad'?'comunidad':n.tipo==='comunicacion'||n.tipo==='evento'?'comunicaciones':'otros';
    ...
    const labels={accion:'Requiere acción',sesiones:'Sesiones y asistencia',comunidad:'Comunidad',comunicaciones:'Comunicaciones',otros:'Otros avisos'};
    const groupCards=[...grouped.values()].sort((a,b)=>(a.key==='accion'?-1:0)-(b.key==='accion'?-1:0)).map(g=>`<section class="notification-group"><header><div><span class="page-kicker">${esc(labels[g.key]||g.key)}</span><h3>${g.items.length} avisos</h3><small>${g.unread} sin leer</small></div><div class="row-actions">${g.unread?`<button class="btn btn-ghost btn-sm mark-group" data-type="${esc(g.items[0]?.tipo||'general')}" data-category="${esc(g.key)}">Marcar grupo leído</button>`:''}</div></header><div class="quick-list">${g.items.slice(0,8).map(n=>quickRow(icon(n.tipo==='evento'?'calendar':n.tipo==='comunicacion'?'megaphone':n.tipo==='comunidad'?'sparkles':'bell'),n.titulo,n.cuerpo||`${n.tipo} · ${dtFmt(n.creado_en)}`,`${n.ruta?`<a class="btn btn-ghost btn-sm" href="#${esc(n.ruta)}">Ver</a>`:''}${!n.leida?`<button class="btn btn-primary btn-sm mark-read" data-id="${esc(n.id)}">Leída</button>`:badge('Leída','neutral')}`)).join('')}</div>${g.items.length>8?`<small class="muted">+ ${g.items.length-8} avisos más dentro de este grupo</small>`:''}</section>`).join('');
    const unread=items.filter(n=>!n.leida).length;const actionUnread=items.filter(n=>!n.leida&&category(n)==='accion').length;
    setMainHtml(`${pageHeader('Notificaciones','Los avisos push activos del club se reciben al autorizar las notificaciones de Android',`${unread?'<button class="btn btn-primary" id="mark-all-read">Marcar todo leído</button>':''}`,'Cuenta')}<div class="metrics"><div class="metric"><span>Sin leer</span><strong>${unread}</strong><small>avisos agrupados abajo</small></div><div class="metric"><span>Requieren acción</span><strong>${actionUnread}</strong><small>pagos, inscripciones o material</small></div><div class="metric"><span>Grupos</span><strong>${grouped.size}</strong><small>categorías activas</small></div></div><div class="notification-groups">${groupCards||empty('Sin notificaciones')}</div>`);
    ...
    document.getElementById('mark-all-read')?.addEventListener('click',async()=>{try{await repos.notifications.markAll();window.dispatchEvent(new CustomEvent('uw-notifications-changed'));toast('Bandeja marcada como leída');await renderNotifications();}catch(e){setError(e)}});
    document.querySelectorAll('.mark-group').forEach(b=>b.addEventListener('click',async()=>{try{const cat=b.dataset.category;const group=[...grouped.values()].find(g=>g.key===cat);for(const t of [...new Set((group?.items||[]).filter(n=>!n.leida).map(n=>n.tipo))])await repos.notifications.markGroup(t);window.dispatchEvent(new CustomEvent('uw-notifications-changed'));toast('Grupo marcado como leído');await renderNotifications();}catch(e){setError(e)}}));
