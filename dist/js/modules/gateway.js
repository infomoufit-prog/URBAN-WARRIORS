...
const directTypes=[
  ...
  {id:'competidor',label:'Competidor',icon:'fighter',accent:'#E21D2D',description:'Perfil preparado para una fase posterior del MVP. Su desarrollo se conserva, pero el alta permanece temporalmente cerrada.',disabled:true,benefits:['Insignia KOMBAX','Perfil deportivo avanzado','Trayectoria y oportunidades']},
    ...
    <section class="gateway-hero premium-surface gateway-brand-stage">
      ...
      <div class="gateway-paths" role="group" aria-label="Accesos KOMBAX">
        ...
        <button class="gateway-path" id="gateway-direct" type="button">
          ...
          <span class="gateway-path-copy"><em>IDENTIDAD GLOBAL</em><strong>Solicitar o gestionar un perfil KOMBAX</strong><small>Club, Marca o Federación. Competidor se activará en una fase posterior del MVP.</small></span>
          <span class="gateway-path-arrow">${icon('arrowUpRight',{size:22})}</span>
        </button>
      </div>
...
function profileEditor(type,{profile=null,onBack,memberProfiles=[]}={}){
  openForm({
    ...
    onSubmit:async v=>{
      ...
      await renderDirectProfileHub({onBack,pendingType:type});
    }
  });
}
...
export async function renderDirectProfileHub({onBack,pendingType=''}={}){
  ...
  try{
    ...
    if(pendingType&&directTypes.some(t=>t.id===pendingType&&!t.disabled)&&['marca','federacion'].includes(pendingType))profileEditor(pendingType,{onBack,memberProfiles});
...
function chooseProfileType({onBack,memberProfiles=[]}={}){
  ...
  const {wrap}=openDetail({title:'Solicitar perfil oficial KOMBAX',subtitle:'Durante el MVP puedes solicitar Club, Marca o Federación. Competidor queda preparado para una fase posterior.',width:'820px',body:`<div class="kx-type-picker">${available.map(t=>`<button type="button" data-kx-pick="${esc(t.id)}"><span>${featureIcon(t.icon,{size:42})}</span><strong>${esc(t.label)}</strong><small>${esc(t.description)}</small><em>${(t.benefits||[]).map(x=>`✓ ${esc(x)}`).join(' · ')}</em></button>`).join('')}</div>`});
...
export function renderDirectProfiles({onBack}){
  ...
  setAppHtml(`<main class="kombax-gateway direct-mode gateway-premium" data-kombax-view="profiles">
    ...
    <section class="gateway-directory premium-surface">
      ...
      <header><span class="gateway-eyebrow">IDENTIDAD KOMBAX</span><h1>Elige el tipo de perfil</h1><p>Cuenta, perfil, verificación y capacidades son capas separadas. Durante el MVP están disponibles Club, Marca y Federación. Competidor, Profesional y Espectador permanecen reservados temporalmente.</p></header>
