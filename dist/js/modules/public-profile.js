...
async function contact(profile){
  ...
  openForm({title:`Contactar con ${profile.nombre_publico}`,subtitle:'Contacto KOMBAX: solicitud profesional y, si se acepta, hasta 20 mensajes totales de solo texto.',fields:[{name:'from',label:'Solicitar como',type:'select',required:true,options:eligible.map(x=>({value:x.id,label:x.identity_label||x.nombre_publico}))},{name:'motivo',label:'Motivo',type:'select',required:true,options:[{value:'entrenamiento',label:'Entrenamiento'},{value:'competicion',label:'Competición'},{value:'evento',label:'Evento'},{value:'colaboracion',label:'Colaboración'},{value:'patrocinio',label:'Patrocinio'},{value:'informacion',label:'Información'},{value:'otro',label:'Otro'}]},{name:'mensaje',label:'Primer mensaje',type:'textarea',required:true,full:true,rows:5,maxLength:500,help:'Entre 10 y 500 caracteres. Cuenta como mensaje 1/20. Sin imágenes, vídeos, audios ni archivos.'}],submitText:'Enviar solicitud',onSubmit:async v=>{await repos.kombaxSocial.contact(v.from,profile.id,v.motivo,v.mensaje);toast('Solicitud enviada · mensaje 1/20');}});
}
...
export async function renderOwnKombaxProfilePage(socialId,{extraHtml='',bindExtra}={}){
  ...
  }catch(error){setError(error);setMainHtml(`${pageHeader('Mi perfil')}<div class="empty"><strong>No se pudo abrir tu perfil KOMBAX</strong><p>${esc(error.message||'Inténtalo de nuevo.')}</p></div>`);return null;}
}
