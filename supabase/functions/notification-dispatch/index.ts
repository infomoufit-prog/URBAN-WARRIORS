...
async function firebaseAccessToken(account: FirebaseServiceAccount): Promise<string> {
  ...
  if (!response.ok || !payload.access_token) throw new Error(`No se pudo obtener token FCM: ${JSON.stringify(payload)}`)
...
async function sendFcm(account: FirebaseServiceAccount, accessToken: string, token: string, notification: Json) {
  ...
  const response = await fetch(`https://fcm.googleapis.com/v1/projects/${account.project_id}/messages:send`, {
    ...
    body: JSON.stringify({
      message: {
        token,
        notification: { title: String(notification.titulo || 'Urban Warriors'), body: String(notification.cuerpo || '') },
        data: { route, payload: JSON.stringify(notification.datos || {}) },
        android: { priority: 'high', notification: { channel_id: 'urban_warriors_alerts' } },
...
Deno.serve(async (request) => {
  try {
    ...
    // Convierte publicaciones programadas vencidas en publicaciones visibles y crea sus avisos internos.
    const { data: scheduledPublished, error: scheduledError } = await supabase.rpc('publicar_comunicaciones_programadas', { p_ahora: new Date().toISOString() })
    if (scheduledError) throw scheduledError
    const { data: classReminders, error: classError } = await supabase.rpc('generar_recordatorios_clase', { p_ahora: new Date().toISOString(), p_horas: 3 })
    if (classError) throw classError

    const firebaseRaw = Deno.env.get('FIREBASE_SERVICE_ACCOUNT_JSON')
    if (!firebaseRaw) {
      return Response.json({ ok: true, firebase_configured: false, scheduled_published: scheduledPublished || 0, class_reminders: classReminders || 0, sent: 0, errors: 0 })
    }
    ...
    let notificationQuery = supabase
      .from('notificaciones')
      .select('id,club_id,perfil_id,rol_destino,audiencia,titulo,cuerpo,ruta,datos,push_intentos,programada_para')
    ...
    let sent = 0
    let errors = 0
    const dueNotifications = (notifications || []).filter((notification) => !notification.programada_para || new Date(notification.programada_para).getTime() <= Date.now())
    for (const notification of dueNotifications) {
      ...
      if (!notification.perfil_id && (notification.rol_destino || notification.audiencia === 'todos')) {
        ...
        for (const member of members || []) recipientIds.add(member.perfil_id)
      }
      ...
      if (ids.length) {
        const { data: devices, error: deviceError } = await supabase
          .from('dispositivos_push').select('id,token').eq('club_id', notification.club_id).eq('activo', true).in('perfil_id', ids)
      ...
      for (const device of tokens) {
        try {
          await sendFcm(account, accessToken, device.token, notification as Json)
          delivered = true
          sent += 1
        } catch (error) {
          errors += 1
      ...
      await supabase.from('notificaciones').update({
        ...
        push_error: itemErrors.length ? itemErrors.join(' | ').slice(0, 2000) : (tokens.length ? null : 'Sin dispositivos push registrados')
      }).eq('id', notification.id)
    }

    return Response.json({ ok: true, firebase_configured: true, scheduled_published: scheduledPublished || 0, class_reminders: classReminders || 0, notifications: dueNotifications.length, sent, errors })
  } catch (error) {
    console.error(error)
