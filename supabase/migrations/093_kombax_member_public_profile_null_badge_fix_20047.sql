...
-- 20.047: an unverified public identity (notably Member) must not collapse
-- to SQL NULL simply because it has no KOMBAX badge. Badge eligibility and
-- public-profile availability are separate concerns.
...
begin
  ...
  v:=jsonb_set(v,'{badge_type}',coalesce(to_jsonb(v_badge),'null'::jsonb),true);
