-- Rollback conservador: restaura el gateway anterior y conserva los metadatos ya guardados.
begin;
do $rollback$
declare v_definition text;
begin
  if to_regprocedure('public.app_mutate_v160_pre_media_028(text,jsonb,uuid)') is null then
    raise exception 'Rollback 028 no disponible';
  end if;
  select pg_get_functiondef('public.app_mutate_v160_pre_media_028(text,jsonb,uuid)'::regprocedure) into v_definition;
  v_definition:=regexp_replace(v_definition,
    'CREATE OR REPLACE FUNCTION public\.app_mutate_v160_pre_media_028\(',
