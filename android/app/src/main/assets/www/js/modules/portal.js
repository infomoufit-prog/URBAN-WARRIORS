...
async function loadPortal(){
  const [members,enrollments,disciplines,grades,groups,schedules,sessions,attendance,progressRows,fees,tracking,documents,graduations,notifs]=await Promise.all([
    repos.portal.visibleMembers(),repos.portal.enrollments(),repos.catalog.disciplines(),repos.catalog.grades(),repos.groups.list(),repos.portal.schedules(),repos.portal.sessions(),repos.portal.attendance(),repos.progress.list(),repos.portal.fees(),repos.portal.tracking(),repos.portal.documents(),repos.portal.graduations(),repos.portal.notifications()
  ]);
  const member=activeMember(members);return {members,member,enrollments,disciplines,grades,groups,schedules,sessions,attendance,progressRows,fees,tracking,documents,graduations,notifs};
}
...
export async function renderPortalSchedule(){
  ...
  try{
    const d=await loadPortal();if(!d.member){setMainHtml(`${pageHeader('Horarios')} ${empty('Sin alumno vinculado')}`);return;}const rel=enrollmentSummary(d,d.member);const gids=new Set(rel.links.map(x=>x.grupo_id));const sched=d.schedules.filter(x=>gids.has(x.grupo_id));const sessions=d.sessions.filter(x=>gids.has(x.grupo_id)&&String(x.fecha)>=isoDate()&&x.estado!=='cancelada').slice(0,20);
    ...
    setMainHtml(`${profileSwitcher(d.members,d.member.id)}${pageHeader('Horarios y acceso','Tus clases habituales y próximas sesiones','<button class="btn btn-primary" id="checkin-now">Registrar acceso</button>','Mi actividad')}${card('Horario habitual',scheduleRows.length?table(['Grupo','Día','Hora'],scheduleRows):empty('Sin horario asignado'))}${card('Próximas sesiones',sessions.length?sessions.map(s=>quickRow(String(s.fecha).slice(8,10),d.groups.find(g=>g.id===s.grupo_id)?.nombre||'Clase',`${dateFmt(s.fecha)} · ${String(s.hora_inicio||'').slice(0,5)} · ${s.estado}`,s.fecha===isoDate()?`<button class="btn btn-primary btn-sm check-session" data-id="${esc(s.id)}">Check-in</button>`:'')).join(''):empty('Sin sesiones próximas'))}`);
    memberBind(d.members,renderPortalSchedule);document.getElementById('checkin-now')?.addEventListener('click',()=>openCheckin(d,d.member));document.querySelectorAll('.check-session').forEach(b=>b.addEventListener('click',()=>openForm({title:'Check-in',subtitle:d.groups.find(g=>g.id===d.sessions.find(s=>s.id===b.dataset.id)?.grupo_id)?.nombre||'',fields:[{name:'codigo',label:'Código de acceso'}],submitText:'Registrar',onSubmit:async v=>{await repos.portal.checkin(b.dataset.id,d.member.id,v.codigo||'');toast('Acceso registrado');await renderPortalSchedule();}})));
...
export async function renderPortalProfile(){
  ...
  try{
    const d=await loadPortal();if(!d.member){setMainHtml(`${pageHeader('Perfil deportivo')} ${empty('Sin alumno vinculado')}`);return;}const pr=d.progressRows.find(x=>x.socio_id===d.member.id)||{};const total=Number(pr.asistencias_registradas||0),present=Number(pr.asistencias_presentes||0),pct=total?Math.round(present/total*100):0;const docs=d.documents.filter(x=>x.socio_id===d.member.id);const tracks=d.tracking.filter(x=>x.socio_id===d.member.id&&x.visibilidad==='familia');const grads=d.graduations.filter(x=>x.socio_id===d.member.id);
    setMainHtml(`${profileSwitcher(d.members,d.member.id)}${pageHeader('Perfil deportivo',`${d.member.nombre} ${d.member.apellidos||''}`,'','Evolución')}
      ...
      ${card('Documentos',docs.length?docs.map(x=>`<div class="document-row"><div><strong>${esc(x.nombre)}</strong><small>${esc(x.tipo)} · ${dateFmt(x.creado_en)}</small></div><button class="btn btn-ghost btn-sm open-doc" data-id="${esc(x.id)}">Abrir</button></div>`).join(''):empty('Sin documentos disponibles'))}`);
    memberBind(d.members,renderPortalProfile);document.querySelectorAll('.open-doc').forEach(b=>b.addEventListener('click',async()=>{try{const doc=docs.find(x=>x.id===b.dataset.id);const url=await repos.documents.url(doc.storage_path);window.open(url,'_blank','noopener,noreferrer')}catch(e){setError(e)}}));
