...
const clubName=()=>String(state.session?.club?.nombre||'Tu club');
const clubMark=()=>clubName().split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]?.toUpperCase()).join('')||'KX';
let communicationLimit=40;
let materialOrderLimit=60;
let notificationLimit=80;
...
function communicationDetail(x,{can=false,reload=()=>{}}={}){
  ...
  const body=`<article class="publication-detail">
    ${x.imagen_url?`<div class="publication-cover"><img src="${esc(x.imagen_url)}" alt="${esc(x.titulo||'Publicación')}"></div>`:`<div class="publication-cover publication-cover-placeholder"><span>${icon('shield',{size:64})}</span><strong>${esc(clubName().toUpperCase())}</strong></div>`}
...
export async function renderCommunications(){
  ...
  try{
    ...
    document.getElementById('load-more-communications')?.addEventListener('click',()=>{communicationLimit=Math.min(300,communicationLimit+40);renderCommunications();});
...
function productDetail(product,{variants=[],members=[],can=false,canManage=false,reload=()=>{}}={}){
  ...
  const body=`<article class="product-detail">
    <div class="product-detail-media">${product.imagen_url?`<img src="${esc(product.imagen_url)}" alt="${esc(product.nombre)}">`:`<div class="product-detail-placeholder">${icon('shield',{size:70})}<strong>${esc(clubName().toUpperCase())}</strong></div>`}</div>
    <div class="product-detail-info"><div class="page-kicker">${esc(product.categoria||'Material del club')}</div><h2>${esc(product.nombre)}</h2><p>${esc(product.descripcion||'Material oficial del club.')}</p><div class="product-detail-price">${money(product.precio)}</div><div class="product-stock ${stock>0?'in':'out'}">${stock>0?`${stock} unidades disponibles`:'Consulta disponibilidad'}</div>
...
export async function renderMaterial(){
  ...
  try{
    ...
    setMainHtml(`${pageHeader(can?'Material':'Tienda del club',can?'Catálogo, variantes y pedidos':'Material oficial, equipación y solicitudes',`${can?'<button class="btn btn-primary" id="new-material">'+icon('plus',{size:16})+' Nuevo material</button>':''}<button class="btn btn-ghost" id="request-material">${icon('shoppingBag',{size:16})} Solicitar material</button>`,'Contenido')}<section class="store-hero"><div><span class="page-kicker">${esc(clubName())} · Material</span><h2>Equípate para entrenar.</h2><p>Consulta material oficial, tallas y disponibilidad. Envía tu solicitud directamente al club.</p></div><div class="store-hero-mark">${icon('shield',{size:58})}<strong>${esc(clubMark())}</strong></div></section><div class="material-grid">${products||empty('Sin material')}</div><div style="height:18px"></div>${card(canManage?'Pedidos':'Mis pedidos',`${orderRows.length?table(['Fecha','Alumno','Artículo','Cant.','Importe','Estado','Acciones'],orderRows):empty('Sin pedidos','Las solicitudes que realices aparecerán aquí con su estado.')}${orders.length>=materialOrderLimit&&materialOrderLimit<400?'<div class="load-more-wrap"><button class="btn btn-ghost" id="load-more-material-orders">Cargar pedidos anteriores</button></div>':''}`)}`);
    ...
    document.getElementById('load-more-material-orders')?.addEventListener('click',()=>{materialOrderLimit=Math.min(400,materialOrderLimit+60);renderMaterial();});
...
export async function renderNotifications(){
  ...
  try{
    const items=await repos.notifications.list({limit:notificationLimit});
    ...
    setMainHtml(`${pageHeader('Notificaciones','Limpia avisos informativos sin perder tareas pendientes',`${pushAction}${informativeUnread?'<button class="btn btn-primary" id="mark-all-read">Marcar informativas como leídas</button>':''}`,'Cuenta')}<div class="metrics"><div class="metric"><span>Sin leer</span><strong>${unread}</strong><small>${informativeUnread} informativas</small></div><div class="metric"><span>Requieren acción</span><strong>${actionUnread}</strong><small>solo desaparecen al revisarlas</small></div><div class="metric"><span>Grupos</span><strong>${grouped.size}</strong><small>categorías activas</small></div></div>${actionUnread?'<div class="alert alert-warning notification-action-note"><strong>Hay tareas pendientes</strong><span>“Marcar informativas como leídas” nunca oculta estas tareas. Abre “Revisar” para atenderlas.</span></div>':''}<div class="notification-groups">${groupCards||empty('Sin notificaciones')}</div>${items.length>=notificationLimit&&notificationLimit<300?'<div class="load-more-wrap"><button class="btn btn-ghost" id="load-more-notifications">Cargar avisos anteriores</button></div>':''}<p class="muted" style="margin-top:14px">El centro carga el historial de forma progresiva. Los avisos antiguos no se descargan todos de golpe.</p>`);
    bind('.mark-read',async(id,el)=>{el.disabled=true;const pending=repos.notifications.markRead(id);await renderNotifications();try{await pending;toast('Notificación leída y guardada');await renderNotifications();}catch(e){setError(e);await renderNotifications();}});
    document.getElementById('mark-all-read')?.addEventListener('click',async()=>{const pending=repos.notifications.markInformative();await renderNotifications();try{const out=await pending;toast(`${Number(out?.marcadas_informativas??informativeUnread)} avisos informativos marcados como leídos`);await renderNotifications();}catch(e){setError(e);await renderNotifications();}});
    ...
    document.getElementById('load-more-notifications')?.addEventListener('click',()=>{notificationLimit=Math.min(300,notificationLimit+80);repos.notifications.invalidate();renderNotifications();});
