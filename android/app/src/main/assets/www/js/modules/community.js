...
async function enrich(posts){
  let profiles=[];try{profiles=await repos.kombaxSocial.clubDirectory('',200)}catch{}
  const profileMap=new Map((profiles||[]).filter(p=>p.socio_id).map(p=>[p.socio_id,p]));
  return Promise.all(posts.map(async p=>{let mediaUrl='',avatarUrl='',coverUrl='';const socialProfile=p.autor_socio_id?profileMap.get(p.autor_socio_id):null;try{mediaUrl=await repos.community.mediaUrl(p.media_path)}catch{}try{if(socialProfile?.avatar_url)avatarUrl=socialProfile.avatar_url;else if(p.autor_avatar_path)avatarUrl=await repos.settings.avatarUrl(p.autor_avatar_path)}catch{}try{const coverPath=p.portada_manual_path||p.portada_automatica_path;if(coverPath)coverUrl=await repos.community.mediaUrl(coverPath)}catch{}return {...p,mediaUrl,avatarUrl,coverUrl,socialProfile};}));
}
...
export async function renderCommunity({append=false}={}){
  ...
  try{
    ...
    const cards=posts.map(p=>`<article class="community-post ${p.estado==='oculta'?'community-hidden':''}">
      <header class="community-author ${p.socialProfile?'community-author-clickable':''}" ${p.socialProfile?`data-author-social="${esc(p.socialProfile.social_id)}" role="button" tabindex="0" aria-label="Ver perfil KOMBAX de ${esc(p.autor_nombre||'miembro')}"`:''}>${p.avatarUrl?`<img src="${esc(p.avatarUrl)}" alt="">`:`<span class="community-avatar-fallback">${esc((p.autor_nombre||'U').slice(0,1).toUpperCase())}</span>`}<div><strong>${esc(p.socialProfile?.apodo_deportivo||p.autor_nombre||'Miembro del club')}</strong><small>${dtFmt(p.creado_en)} · ${daysLeft(p.expira_en)} días restantes${p.socialProfile?' · Ver perfil KOMBAX':''}</small></div>${p.estado==='oculta'?badge('Oculta','warn'):''}</header>
    ...
    setMainHtml(`${pageHeader('Comunidad del Club','Espacio interno del club. Sus publicaciones no aparecen automáticamente en KOMBAX Social.',`<button class="btn btn-ghost" id="community-go-social">${icon('activity',{size:16})} Ir a KOMBAX Social</button><button class="btn btn-ghost" id="community-members">${icon('users',{size:16})} Perfiles</button><button class="btn btn-ghost" id="community-blocked">Bloqueados</button>${canModerate()?'<button class="btn btn-ghost" id="community-reports">Denuncias</button>':''}<button class="btn btn-primary" id="community-new" ${publishDisabled?'disabled':''}>${icon('plus',{size:16})} Publicar</button>`,'Club')}
      <section class="kx-scope-banner internal"><div>${icon('lock',{size:21})}</div><div><small>DESTINO DE PUBLICACIÓN</small><strong>Solo ${esc(state.session?.club?.nombre||'este club')}</strong><span>Para publicar de forma pública en toda la red utiliza KOMBAX Social.</span></div><button type="button" class="btn btn-ghost btn-sm" id="community-go-social-inline">Abrir Social</button></section>
      <section class="community-hero"><div><span class="page-kicker">COMUNIDAD DEL CLUB · <button type="button" class="community-club-name" id="community-club-profile" aria-label="Abrir perfil público de ${esc(state.session?.club?.nombre||'Tu club')}">${esc(String(state.session?.club?.nombre||'Tu club').toUpperCase())}</button></span><h2>Tu club también se vive aquí.</h2><p>Comparte una imagen o un vídeo de hasta 15 segundos. El contenido se elimina automáticamente a los 30 días.</p></div><div class="community-quota"><strong>${quota.used}/${quota.limit}</strong><span>publicaciones este mes</span></div></section>
    ...
    const goSocial=()=>{location.hash='#social';};
    document.getElementById('community-go-social')?.addEventListener('click',goSocial);
    document.getElementById('community-go-social-inline')?.addEventListener('click',goSocial);
    ...
    document.querySelectorAll('[data-author-social]').forEach(el=>{const open=()=>openKombaxPublicProfile(el.dataset.authorSocial);el.addEventListener('click',open);el.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});});
