...
function renderCredentials({onCancel,onSuccess,message=''}){
  ...
  form?.addEventListener('submit',async event=>{
    event.preventDefault();if(!form.reportValidity())return;button.disabled=true;button.textContent='Validando acceso Owner…';errorBox.hidden=true;
    const fd=new FormData(form);
    try{await backend.beginPlatformAdminAccess(fd.get('email'),fd.get('password'));toast('Acceso Owner verificado');onSuccess?.();}
    catch(error){errorBox.hidden=false;errorBox.textContent=humanError(error);button.disabled=false;button.innerHTML=`Abrir Consola Owner ${icon('shieldCheck',{size:17})}`;}
  });
