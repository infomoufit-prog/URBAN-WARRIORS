...
function guardianConsentPanel(){
  ...
  let html='';
  if(pendingMine)html+=`<section class="kombax-social-notice guardian"><div>${icon('shield',{size:28})}</div><div><strong>Consentimiento del tutor pendiente</strong><p>Tu tutor vinculado debe aprobar el acceso público a KOMBAX Social. Hasta entonces puedes seguir usando Mi club con normalidad.</p></div></section>`;
  else if(approvedMine)html+=`<section class="kombax-social-notice guardian ok"><div>${icon('shieldCheck',{size:28})}</div><div><strong>Consentimiento del tutor aprobado</strong><p>El control adulto está registrado. Las protecciones de menores siguen activas y el chat personal permanece bloqueado hasta los 18 años.</p></div></section>`;
...
async function openContactThread(contact){
  ...
  const msgHtml=m=>`<article class="kx-contact-message ${m.propio?'own':'other'}" data-message-id="${esc(m.id)}" data-ordinal="${Number(m.ordinal)||0}"><small>${esc(m.autor_nombre)} · ${dtFmt(m.creado_en)}</small><p>${esc(m.texto).replace(/\n/g,'<br>')}</p><footer>${receiptHtml(m)}${!m.propio&&!/^\[Mensaje retirado/.test(String(m.texto||''))?`<button type="button" class="kx-message-report" data-kx-message-report="${esc(m.id)}" aria-label="Denunciar mensaje">${icon('alert',{size:13})} Denunciar</button>`:''}</footer></article>`;
...
async function renderContacts(){
  ...
  try{
    ...
    document.getElementById('load-more-contacts')?.addEventListener('click',()=>{contactLimit=Math.min(200,contactLimit+50);renderContacts();});
...
async function renderSafety(){
  setMainHtml(`<div class="kombax-social-page">${socialHeader()}${pageHeader('Seguridad y alcance','Controles claros para una comunidad responsable.','','KOMBAX Social')}${identitySwitcher()}${tabBar()}${guardianConsentPanel()}<div class="kombax-safety-grid"><article>${icon('shieldCheck',{size:30})}<strong>Separación real</strong><p>KOMBAX Social no muestra expedientes, cuotas, asistencia, teléfonos, correos, domicilios ni relaciones familiares.</p></article><article>${icon('message',{size:30})}<strong>Contacto KOMBAX</strong><p>El chat solo se habilita tras aceptar una solicitud con motivo previo. El historial permanece abierto y es solo texto: sin imágenes, vídeos, audios ni archivos en esta fase.</p></article><article>${icon('users',{size:30})}<strong>Protección de menores</strong><p>Los perfiles personales menores de 18 requieren consentimiento de un tutor vinculado para activar Social. El contacto privado permanece bloqueado hasta los 18 años.</p></article><article>${icon('alert',{size:30})}<strong>Moderación global</strong><p>Publicaciones, comentarios, perfiles y mensajes concretos pueden denunciarse. Los bloqueos y las acciones de moderación conservan trazabilidad sin alterar la membresía del club.</p></article></div><section id="kx-moderation-console"></section></div>`);bindCommon();
