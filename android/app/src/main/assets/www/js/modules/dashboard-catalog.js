...
export async function renderDashboard(){
  ...
  try{
    ...
    const unreadCount=Number(d.notificationSummary?.club_unread_items||0);const pendingPre=d.pre.filter(x=>['enviada','en_revision','pendiente_documentacion','lista_espera'].includes(x.estado));const pendingPayments=d.payments.filter(x=>x.estado_validacion==='pendiente');
    ...
    if(role==='economia'){
      ...
      const todaySessions=future.filter(x=>String(x.fecha)===today);const myGroups=activeGroups;
      setMainHtml(`${hero({kicker:'Hoy',title:`Hola, ${state.session?.nombre||'Warrior'}.`,body:'Tus clases, asistencia y seguimiento en un panel pensado para moverte rápido desde el tatami.',actions:'<button class="btn btn-primary" data-nav="attendance">Pasar asistencia</button><button class="btn btn-ghost" data-nav="members">Mis alumnos</button>',sideValue:String(todaySessions.length),sideLabel:'clases programadas hoy',dark:true})}
      ...
      setMainHtml(`${hero({kicker:'Comunicación',title:'La voz del club, en una sola bandeja.',body:'Publica noticias, eventos y avisos manteniendo una experiencia coherente para familias, alumnado y equipo.',actions:'<button class="btn btn-primary" data-nav="communications">Gestionar publicaciones</button><button class="btn btn-ghost" data-nav="notifications">Ver avisos</button>',sideValue:String(unreadCount),sideLabel:'notificaciones sin leer',dark:true})}
        <div class="metrics">${metric('Avisos sin leer',unreadCount)}${metric('Sesiones próximas',future.length)}${metric('Grupos activos',activeGroups.length)}${metric('Socios activos',activeMembers.length)}</div>`);
    }else{
      const secretary=role==='secretaria';const coordination=role==='coordinacion';const actionItems=[
      ...
      setMainHtml(`${hero({kicker:secretary?'Secretaría · Operación diaria':coordination?'Coordinación · Gestión operativa':'Gestor de la app · Panel global',title:secretary?'Todo lo pendiente, bajo control.':coordination?'Coordina el club desde un único panel.':'Todo el gimnasio, en una sola app.',body:secretary?'Prioriza solicitudes, documentación, matrículas, pagos y clases del día.':coordination?'Alumnos, grupos, sesiones, finanzas, publicaciones y documentación con permisos operativos amplios.':'Inscripciones, clases, asistencia, finanzas y comunicaciones con una visión global del club.',actions:'<button class="btn btn-primary" data-nav="members">Gestionar alumnos</button><button class="btn btn-ghost" data-nav="attendance">Pasar asistencia</button>',sideValue:String(activeMembers.length),sideLabel:'socios activos'})}
...
export async function renderCatalog(){
  ...
  try{
    ...
    bind('.delete-discipline',id=>confirmDialog('Eliminar disciplina','Solo se elimina si nunca se ha utilizado. Si tiene grupos, matrículas, sesiones o graduaciones, desactívala y conserva el histórico.',async()=>{await repos.catalog.deleteDiscipline(id);toast('Disciplina eliminada');await reload();},{confirmText:'Eliminar',danger:true}));
    ...
    document.getElementById('new-grade')?.addEventListener('click',()=>openGrade());bind('.edit-grade',id=>openGrade(grades.find(x=>x.id===id)));bind('.add-grade',id=>openGrade({disciplina_id:id,activo:true,orden:grades.filter(g=>g.disciplina_id===id).length+1}));bind('.delete-grade',id=>confirmDialog('Eliminar grado','Solo se elimina si nunca se ha utilizado. Si tiene graduaciones asociadas, desactívalo y conserva el histórico.',async()=>{await repos.catalog.deleteGrade(id);toast('Grado eliminado');await reload();},{confirmText:'Eliminar',danger:true}));
