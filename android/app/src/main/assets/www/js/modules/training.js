...
export async function renderSessions(){
  ...
  try{
    const [sessions,groups]=await Promise.all([repos.sessions.list(),repos.groups.list()]);const can=has(state.session,'session');
    const rows=sessions.map(s=>`<tr><td>${dateFmt(s.fecha)}<br><small>${esc(String(s.hora_inicio||'').slice(0,5))}${s.hora_fin?`–${esc(String(s.hora_fin).slice(0,5))}`:''}</small></td><td><strong>${esc(groups.find(g=>g.id===s.grupo_id)?.nombre||'—')}</strong></td><td>${esc(s.monitor_nombre||'—')}</td><td>${badge(s.estado,s.estado==='completada'?'ok':s.estado==='cancelada'?'danger':s.estado==='en_curso'?'warn':'neutral')}</td><td>${can?`<button class="btn btn-ghost btn-sm edit-session" data-id="${esc(s.id)}">Editar</button>`:''}</td></tr>`);
    setMainHtml(`${pageHeader('Sesiones','Programación de entrenamientos',can?'<button class="btn btn-primary" id="new-session">Nueva sesión</button>':'')}${card('Sesiones',rows.length?table(['Fecha','Grupo','Monitor','Estado','Acciones'],rows):empty('Sin sesiones'))}`);
...
export async function renderAttendance(){
  ...
  try{
    const [sessions,groups,members,enrollments,attendance]=await Promise.all([repos.sessions.list(),repos.groups.list(),repos.members.list(),repos.members.enrollments(),repos.sessions.attendance()]);
    ...
    setMainHtml(`${pageHeader('Asistencia','Registro por sesión')}${card('Selecciona sesión',`<div class="field"><label>Sesión</label><select id="attendance-session"><option value="">Selecciona</option>${sessionOptions.map(o=>`<option value="${esc(o.value)}" ${o.value===selected?'selected':''}>${esc(o.label)}</option>`).join('')}</select></div><div id="attendance-board" style="margin-top:16px"></div>`)}`);
    ...
    const draw=(sid)=>{
      ...
      const ids=new Set(enrollments.filter(e=>e.grupo_id===ses.grupo_id&&e.activa).map(e=>e.socio_id));const list=members.filter(m=>ids.has(m.id));
      const rows=list.map(m=>{const a=attendance.find(x=>x.sesion_id===sid&&x.socio_id===m.id);return `<tr><td><strong>${esc(m.apellidos)}, ${esc(m.nombre)}</strong></td><td>${badge(a?.estado||'pendiente',a?.estado==='presente'?'ok':a?.estado==='ausente'?'danger':a?.estado==='retraso'?'warn':'neutral')}</td><td><div class="row-actions">${canAttendance?['presente','ausente','ausencia_justificada','retraso'].map(st=>`<button class="btn btn-ghost btn-sm set-att" data-id="${esc(m.id)}" data-status="${st}">${esc(st)}</button>`).join(''):''}${canCheckin?`<button class="btn btn-primary btn-sm do-checkin" data-id="${esc(m.id)}">Check-in</button>`:''}</div></td></tr>`});
      board.innerHTML=rows.length?table(['Alumno','Estado','Acciones'],rows):empty('Sin alumnos matriculados en este grupo');
