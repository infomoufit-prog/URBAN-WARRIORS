...
export async function renderSessions(){
  ...
  try{
    ...
    setMainHtml(`${pageHeader('Sesiones','Series semanales, excepciones y confirmaciones previas',headerActions)}${card('Sesiones recurrentes',seriesRows.length?table(['Grupo','Días','Hora','Monitor','Estado','Acciones'],seriesRows):empty('Sin series recurrentes','Crea una clase semanal y Urban Warriors mantendrá 12 semanas futuras generadas automáticamente.'))}${card('Calendario de sesiones',rows.length?table(['Fecha','Grupo','Monitor / sala','Estado','Reservas','Acciones'],rows):empty('Sin sesiones'))}`);
