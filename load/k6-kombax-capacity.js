...
export default function(){
  ...
  if(tokens.length&&clubs.length){const i=__VU%Math.min(tokens.length,clubs.length),token=tokens[i],club=clubs[i];const feed=postRpc('app_kombax_social_feed_v041',{p_cursor:null,p_cursor_id:null,p_limit:20},token);authenticatedLatency.add(feed.timings.duration);errors.add(!check(feed,{'social feed 200':r=>r.status===200}));const notifications=postRpc('app_notificaciones_centro_v037',{p_club_id:club,p_limit:100},token);authenticatedLatency.add(notifications.timings.duration);errors.add(!check(notifications,{'notifications 200':r=>r.status===200}));}
  sleep(1);
}
