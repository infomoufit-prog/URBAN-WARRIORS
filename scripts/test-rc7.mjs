...
assert(cfg.includes("version: '2.0.0-rc.9'")&&cfg.includes('build: 20009')&&cfg.includes("app_diagnostico_final_v165"),'RC9 conserva regresión RC7 / v164');
...
assert(sql.includes("tiene_rol_club(p_club_id,'direccion','secretaria','comunicacion')")&&perms.includes("communication:['direccion','coordinacion','secretaria','comunicacion']")&&app.includes("'communications','material','documents'"),'Secretaría entra en gestión editorial');
...
assert(admin.includes('E2E_RC9_')&&admin.includes('repos.members.forceDelete')&&admin.includes('repos.groups.forceDelete')&&admin.includes('publicacionEImagen'),'certificación final limpia sus propios datos');
