...
if(build<20040){for(const text of ['Solicitudes de contacto','No existe chat'])assert(module.includes(text),`interfaz histórica incluye “${text}”`);}else{
  assert(module.includes('Contacto KOMBAX'),'evolución de Contacto KOMBAX conserva su identidad');
  assert(/solo texto/i.test(module),'evolución de Contacto KOMBAX conserva chat solo texto');
  assert(module.includes('historial permanece abierto')||module.includes('chat de texto abierto'),'evolución 20059 refleja chat abierto sin límite artificial');
