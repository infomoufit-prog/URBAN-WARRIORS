...
assert(media.includes('duration>15.2')&&media.includes("startsWith('video/')"),'cliente valida vídeo máximo 15 segundos');
...
assert(comms.includes('markInformative')&&comms.includes('markGroup')&&comms.includes('Requiere acción'),'notificaciones agrupadas y acciones masivas preservadas con protección de tareas accionables');
assert(sql.includes('create table if not exists public.preferencias_notificacion'),'tabla histórica de preferencias preservada sin destrucción');
assert(sql.includes('create or replace view public.v_estado_cuenta_socio')&&finance.includes('Historial financiero')&&finance.includes("metric('Cobrado'")&&finance.includes('pendingValidation'),'Tesorería conserva estado de cuenta y amplía métricas operativas');
const currentBuild=Number(cfg.match(/build:\s*(\d+)/)?.[1]||0);
if(currentBuild<20048)assert(repos.includes("uploadAvatar")&&admin.includes('Foto de perfil')&&portal.includes('Foto de perfil')&&components.includes('data-session-avatar'),'perfil privado con avatar integrado');
else assert(repos.includes("uploadAvatar")&&admin.includes('Foto de perfil')&&portal.includes('Foto privada de acceso')&&portal.includes('No forma parte de tu perfil público KOMBAX')&&components.includes('data-session-avatar'),'foto privada RC10 preservada como capa separada del perfil KOMBAX');
...
assert(help.includes('Manual interactivo')&&help.includes('Cartel de descarga KOMBAX')&&help.includes("['direccion','coordinacion','secretaria']"),'Ayuda interactiva y recursos restringen cartel al equipo autorizado');
...
assert(pkg.includes('test-rc10.mjs')&&Number(gradle.match(/versionCode\s+(\d+)/)?.[1])>=20021&&gradle.includes("versionName '2.0.0-rc.13'")&&/rc13-20\d{3}/.test(sw),'tests base RC10 preservan la cadena y versionado monótono');
