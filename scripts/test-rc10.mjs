...
assert(cfg.includes("version: '2.0.0-rc.10'")&&cfg.includes('build: 20010')&&cfg.includes("app_diagnostico_final_v166"),'versionado RC10 / diagnóstico v166');
...
assert(sql.includes('create table if not exists public.preferencias_notificacion')&&dispatch.includes('push_comunidad')&&reminders.includes('push_finanzas'),'preferencias push por categoría respetadas en Edge Functions');
assert(sql.includes('create or replace view public.v_estado_cuenta_socio')&&finance.includes('Estado de cuenta')&&finance.includes('Cobrado validado')&&finance.includes('Por validar'),'Tesorería incluye estado de cuenta y métricas operativas');
...
assert(pkg.includes('test-rc10.mjs')&&gradle.includes('versionCode 20010')&&gradle.includes("versionName '2.0.0-rc.10'")&&sw.includes('rc10-20010'),'tests, Android y service worker versionados RC10');
