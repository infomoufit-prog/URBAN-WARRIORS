...
const js=(await files(resolve(web,'js'))).filter(f=>f.endsWith('.js'));const texts=Object.fromEntries(await Promise.all(js.map(async f=>[relative(web,f),await readFile(f,'utf8')])));
...
assert(/versionCode\s+20012/.test(gradle)&&/versionName '2\.0\.0-rc\.12'/.test(gradle),'Android versionado 2.0.0-rc.12 / 20012');
...
assert(mainActivity.includes('UrbanWarriorsApp/2.0.0-rc.12'),'User-Agent Android acompaña versión RC10');
