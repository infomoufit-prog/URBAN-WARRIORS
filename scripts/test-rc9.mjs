...
assert(app.includes("role==='direccion'||role==='coordinacion'")&&!app.includes("allowed.add('diagnostics')")&&!app.includes("allowed.add('certification')"),'Gestor/Coordinación comparten navegación operativa sin herramientas técnicas en el shell de club');
assert((admin.includes("{value:'coordinacion',label:'Coordinación'}")||admin.includes('TEAM_INVITE_ROLES'))&&admin.includes("m.coord?'Coordinación'")&&admin.includes("isManager=state.session?.rol==='direccion'"),'equipo representa Coordinación como un único rol y solo Gestor puede concederla');
assert(!admin.includes('Herramientas técnicas')&&!admin.includes('#diagnostics')&&!admin.includes('#certification'),'herramientas técnicas quedan fuera del frontend ordinario del club');
