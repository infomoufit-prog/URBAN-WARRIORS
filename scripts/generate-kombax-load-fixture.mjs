...
const clubs=Number(args.clubs||0);
if(![10,50,100].includes(clubs))throw new Error('Usa --clubs=10, --clubs=50 o --clubs=100.');
const output=resolve(args.output||`supabase/fixtures/load/${String(clubs).padStart(3,'0')}_clubs.sql`);

const sql=`-- KOMBAX LOAD FIXTURE · ${clubs} CLUBES · SOLO LOCAL/QA
-- NO EJECUTAR EN PRODUCCIÓN. Requiere habilitación manual de sesión:
--   set app.kombax_load_fixture_enabled = 'on';
-- Volumen nominal: ${clubs} clubes, ${clubs*3} disciplinas, ${clubs*4} grupos,
-- ${clubs*40} socios, ${clubs*112} sesiones, ${clubs*1120} asistencias,
-- ${clubs*20} notificaciones y ${clubs*240} cuotas.
...
do $fixture$
declare
  v_total integer:=${clubs};v_i integer;v_d integer;v_g integer;v_m integer;v_s integer;v_a integer;v_month integer;
...
console.log(`OK fixture ${clubs} clubes -> ${output}`);
