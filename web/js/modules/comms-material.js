...
export async function renderMaterial(){
  ...
  try{
    ...
    const orderRows=orders.map(o=>{const v=variants.find(x=>x.id===o.variante_id);return `<tr><td>${dateFmt(o.creado_en)}</td><td>${esc(members.find(m=>m.id===o.socio_id)?.nombre||'—')}</td><td><strong>${esc(items.find(i=>i.id===o.material_id)?.nombre||'—')}</strong>${v?`<small class="muted">${esc([v.talla,v.color].filter(Boolean).join(' · '))}</small>`:''}</td><td>${esc(o.cantidad)}</td><td>${money(o.importe_total)}</td><td>${badge(orderLabel(o.estado),o.estado==='entregado'?'ok':o.estado==='cancelado'?'danger':o.estado==='preparado'?'warn':'neutral')}</td><td>${canManage?`<button class="btn btn-ghost btn-sm status-order" data-id="${esc(o.id)}">Cambiar</button>`:''}</td></tr>`});
...
export async function renderNotifications(){
  ...
  try{
    const [items,prefsRows]=await Promise.all([repos.notifications.list(),repos.notifications.preferences().catch(()=>[])]);const prefs=prefsRows?.[0]||{push_general:true,push_finanzas:true,push_sesiones:true,push_comunidad:false};
    ...
    document.getElementById('notif-preferences')?.addEventListener('click',()=>openForm({title:'Preferencias de avisos push',subtitle:'Los avisos esenciales siguen visibles dentro de la app aunque desactives una categoría push.',fields:[{name:'push_general',label:'Avisos generales',type:'checkbox',value:prefs.push_general!==false},{name:'push_finanzas',label:'Cuotas y pagos',type:'checkbox',value:prefs.push_finanzas!==false},{name:'push_sesiones',label:'Sesiones, cancelaciones y cambios',type:'checkbox',value:prefs.push_sesiones!==false},{name:'push_comunidad',label:'Nuevas publicaciones de Comunidad',type:'checkbox',value:prefs.push_comunidad===true}],submitText:'Guardar preferencias',onSubmit:async v=>{await repos.notifications.savePreferences(v);toast('Preferencias guardadas');await renderNotifications();}}));
