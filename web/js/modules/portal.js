...
export async function renderPortalProfile(){
  ...
  try{
    const d=await loadPortal();if(!d.member){setMainHtml(`${pageHeader('Perfil deportivo')} ${empty('Sin alumno vinculado')}`);return;}const pr=d.progressRows.find(x=>x.socio_id===d.member.id)||{};const total=Number(pr.asistencias_registradas||0),present=Number(pr.asistencias_presentes||0),pct=total?Math.round(present/total*100):0;const docs=d.documents.filter(x=>x.socio_id===d.member.id&&x.estado!=='archivado'&&x.estado!=='sustituido');const tracks=d.tracking.filter(x=>x.socio_id===d.member.id&&x.visibilidad==='familia');const grads=d.graduations.filter(x=>x.socio_id===d.member.id);
    ...
    memberBind(d.members,renderPortalProfile);document.querySelectorAll('.open-doc').forEach(b=>b.addEventListener('click',async()=>{try{const doc=docs.find(x=>x.id===b.dataset.id);const url=await repos.documents.url(doc.storage_path);window.open(url,'_blank','noopener,noreferrer')}catch(e){setError(e)}}));document.querySelectorAll('.download-portal-doc').forEach(b=>b.addEventListener('click',async()=>{try{const doc=docs.find(x=>x.id===b.dataset.id);await downloadPortalDocument(doc);toast('Descarga iniciada')}catch(e){setError(e)}}));
